...
contract ClickButton {
    ...
    mapping(uint => uint[]) RewardedBySession;
    mapping(uint => mapping(uint => Round)) RoundBySession;

    function getInfo() constant public returns (uint totalSessions, uint totalRounds, uint rewarded) {
        return (SessionTip, RoundTip, RewardedBySession[SessionTip].length);
    }
    ...
    function saveReward(uint roundIndex, uint reward) public {
        ...
        //RewardedBySession[SessionTip][RewardedBySession[SessionTip].length] = roundIndex;
    }
    ...
    function getRoundByRound(uint sessionIndex, uint roundIndex) public constant returns(uint session, uint id, uint reward, uint count, uint eventCount, bool writeLocked) {
        Round storage _round = RoundBySession[sessionIndex][roundIndex];
        return (sessionIndex, _round.Id, _round.Reward, RoundTip, _round.EventTip, _round.Locked);
    }

    function getEventByRound(uint sessionIndex, uint roundIndex, uint eventIndex) public constant returns(uint session, uint id, uint x, uint y, uint buttonmask, uint count, bool writeLocked) {
        Round storage _round = RoundBySession[sessionIndex][roundIndex];
        PointerEvent storage _event = _round.EventByIndex[eventIndex];
        return (sessionIndex, _event.Id, _event.X, _event.Y, _event.Buttonmask, _round.EventTip, _event.Locked);        
    }
    ...
    function withRewards(uint sessionIndex) public returns (uint[]) {
        return RewardedBySession[sessionIndex];
    }
